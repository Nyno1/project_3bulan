...
<main class="pb-20 bg-white">
    <!-- Hero Section -->
    <section id="beranda" class="relative w-full bg-gradient-to-r from-blue-50 via-white to-orange-50 pb-24 overflow-hidden">
        <!-- efek bulat -->
        ...
        <!-- konten dengan padding-top supaya tidak nabrak navbar -->
        <div class="relative z-10 max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 items-center px-6 pt-28">
    ...
    <!-- Galeri -->
    <section class="max-w-7xl mx-auto px-4 py-20" id="galeri" aria-label="Galeri">
        <div class="text-center mb-14" data-aos="fade-up">
        <h2 class="relative inline-block text-3xl font-bold text-gray-700 before:content-[''] before:absolute before:-top-4 before:left-1/2 
            before:-translate-x-1/2 before:w-20 before:h-1 
            before:bg-gradient-to-r before:from-blue-400 before:to-orange-400">
            Ikuti Kegiatan Terbaru
        </h2>
        <p class="text-gray-600 mt-2">Dokumentasi kegiatan dan sertifikat siswa</p>
    </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10" data-aos="fade-up" data-aos-delay="80">
            @php
                $galeriItems = [
            ...
            @foreach($galeriItems as $i => $item)
                <article class="bg-white border border-gray-200 rounded-xl shadow-md overflow-hidden group transition hover:shadow-xl" data-aos="zoom-in" data-aos-delay="{{ 80 + $i*60 }}">
                    <div class="overflow-hidden">
                        <img src="{{ asset('images/' . $item['img']) }}" alt="{{ $item['title'] }}" class="w-full h-64 object-cover group-hover:scale-105 transition duration-500">
                    </div>
                    <div class="p-5">
